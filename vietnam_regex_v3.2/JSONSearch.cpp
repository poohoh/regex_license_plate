...
// 4. search vehicle_type
result_struct JSONSearch::search_vehicle_type(const std::string &input_string) const {
    result_struct vehicle_type_result("vehicle_type");

    if (!json_data.contains("vehicle_type"))  // JSON 파일에 카테고리가 없는 경우
    {
        vehicle_type_result.value = "No vehicle_type key in JSON file.";
        return vehicle_type_result;
    }

    const nlohmann::json *current = &json_data["vehicle_type"];
    ...
    for (const auto&[regex, name] : current->items())
    {
        ...
        if (std::regex_search(input_string, pattern))
        {
            vehicle_type_result.value = name;
            return vehicle_type_result;
        }
    }
